# Управление Программными Пакетами

**Языки:** [English](README.md) · [简体中文](README_zh.md) · [Español](README_es.md) · [Français](README_fr.md) · [Deutsch](README_de.md) · [日本語](README_ja.md) · [Русский](README_ru.md) · [한국어](README_ko.md) · [Português](README_pt.md)

<p align="center">
  <a href="https://labex.io/ru/courses/software-package-management">
    <img src="https://course-cover.labex.io/software-package-management.png?lang=ru" alt="Управление Программными Пакетами">
  </a>
</p>

Изучите управление программными пакетами. Этот модуль охватывает менеджеры пакетов APT и DNF, поиск пакетов, установку и удаление программного обеспечения, системные обновления и апгрейды, управление репозиториями. Вы освоите эти важные навыки Linux с помощью практических лабораторных работ и реальных задач.

[Начать курс на LabEx](https://labex.io/ru/courses/software-package-management)

## Упражнения

|   Индекс | Название                                      | Сложность   | Практика                                                                                                                                             |
|----------|-----------------------------------------------|-------------|------------------------------------------------------------------------------------------------------------------------------------------------------|
|        1 | Менеджеры пакетов APT и DNF                   | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/package-managers-apt-and-dnf-636460?course=software-package-management'>Начать лабораторию</a>     |
|        2 | Поиск пакетов                                 | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/searching-for-packages-636461?course=software-package-management'>Начать лабораторию</a>           |
|        3 | Установка и удаление программного обеспечения | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/installing-and-removing-software-636458?course=software-package-management'>Начать лабораторию</a> |
|        4 | Обновление и модернизация системы             | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/system-updates-and-upgrades-636462?course=software-package-management'>Начать лабораторию</a>      |
|        5 | Управление репозиториями                      | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/managing-repositories-636459?course=software-package-management'>Начать лабораторию</a>            |
|        6 | Управление жизненным циклом веб-сервера       | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/web-server-setup-636704?course=software-package-management'>Начать испытание</a>                   |

## About LabEx

<div align="left"><p><a href="https://labex.io"><strong>LabEx</strong></a> is a <strong>hands-on learning platform for beginners</strong>.</p><p>Explore <a href="https://labex.io/learn/linux"><strong>Linux</strong></a>, <a href="https://labex.io/learn/devops"><strong>DevOps</strong></a>, <a href="https://labex.io/learn/cybersecurity"><strong>Cybersecurity</strong></a>, and <strong>more</strong> — all directly in your browser.</p><p>Learn step by step through <strong>interactive labs</strong>, <strong>guided exercises</strong>, and <strong>real-world projects</strong>. 🌱<br />No setup, no stress — just practice and grow your skills by doing.</p><br /><p><a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/assets/elements/badges/download-on-the-app-store.svg" alt="Download on the App Store" height="54" /></a>&nbsp;<a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/app-store/marketing/guidelines/images/badge-download-on-the-mac-app-store.svg" alt="Download on the Mac App Store" height="52" /></a></p><br /><p>📖 Need help? Visit our <a href="https://support.labex.io/">Help Center</a> or email info@labex.io</p></div>

